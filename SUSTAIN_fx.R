...
H_j_act_numerator_fx <- function(lambda_vec,par, mue_ij,j){
  numerator_j <- lambda_vec^par["r"]*exp(-lambda_vec* mue_ij[j,])
  return(numerator_j)
}
Eq4to7_fx <- function(H, M_HC, I_unlabeled, I, m, V, lambda_vec, maskedValues){
  ...
  # Equation 5
  H_j_act_numerator <- t(sapply(c(1:nrow(H)),H_j_act_numerator_fx, lambda_vec= lambda_vec,par=par, mue_ij= mue_ij))
  H_j_act_numerator <- rowSums(H_j_act_numerator,na.rm=T)
  #H_j_act_numerator <- rowSums(lambda_vec^par["r"]*exp(-lambda_vec* mue_ij),na.rm=T)
  H_j_act_denom <- sum(lambda_vec^par["r"],na.rm=T)
  ...
  Act_after_competition <- (H_j_act^par["beta"]/sum(H_j_act^par["beta"]))*H_j_act
...
trial_fx <- function( # fixed
  par, trial, block_stimuli, ResponseSet, maskedValues, maskedDim, m, V, 
  ...
  Eq4to7 <- Eq4to7_fx( H=H, M_HC=M_HC, I_unlabeled=I_unlabeled, I=I, m=m, V=V, lambda_vec=lambda_vec, maskedValues= maskedValues)
  ...
  # Equation 8
  Pr_k <- exp(par["d"]* C_zk_out)/sum(exp(par["d"]* C_zk_out))
  ...
  if((i>1)&(t_value_at_Cmax!=1)){
    ...
    # "... and the clusters' activations and outputs are recalculated. The new cluster then becomes the winner...(p.316)"
    Eq4to7 <- Eq4to7_fx( H=H, M_HC=M_HC, I_unlabeled=I_unlabeled, I=I, m=m, V=V, lambda_vec=lambda_vec, maskedValues= maskedValues)
    names(Eq4to7) <- c("H","mue_ij","M_HC","j_winner","H_j_out","C_zk_out")
    H <- Eq4to7$H
    M_HC <- Eq4to7$M_HC
  ...
  delta_H_j_pos_ik <- par["eta"]*(I_pos_ik - H_j_pos_ik)
  H[j_winner,] <- H[j_winner,]+ delta_H_j_pos_ik
  
  # Equation 13 (Adjusting lambdas)
  eq_13_comp1 <- par["eta"]*exp(-lambda_vec*mue_ij[j_winner,])
  ...
  delta_w_j_zk <- par["eta"]*(t_zk-C_zk_out)* H_j_out[j_winner]
...
block_fx <- function( par, agent, block, stimuli, ResponseSet, structure_name, 
  ...
  for(trial in 1:nrow(block_stimuli)){
    ...
    trial_outcome <- trial_fx(par=par,trial=trial, block_stimuli= block_stimuli, ResponseSet = ResponseSet, maskedDim= maskedDim, maskedValues=maskedValues, m=m, V=V, lambda_vec= lambda_vec, H=H, M_HC= M_HC, nStimCorrect= nStimCorrect, i=i)
...
agent_fx <- function( par, agent, nBlocks, termCrit,
                      labelQuery, ResponseSet, structure_name, stimulus_structure, 
                      m, V, feature_coding
){
  # "...32 blocks or ... 4 blocks without an error. ..., a block is defined as the presentation of each item in a random order."(p. 319)
  ...
  # "Initially, lambda_i is set to be broadly tuned with a value of 1. The value of 1 is chosen because the maximal distance mue_ij is 1 and the optimal setting of lambda_i for this case is 1 (i.e., Equation 13 equals zero). Under this scheme, lambda_i cannot become less than 1 but can become more narrowly tuned." (p. 316)
  evolved_lambdas <- rep(1,m)
  # ??????? Does the model change its behavior if label-dimension is cancelled from calculating activation values???????
  ...
  for(block in 1: nBlocks){
    ...
    blockOutcome <- block_fx(par=par, agent=agent, block=block, stimuli=stimuli, ResponseSet=ResponseSet, structure_name= structure_name, m=m, maskedDim=maskedDim, maskedValues=maskedValues, lambda_vec=evolved_lambdas, H=evolved_H, M_HC=evolved_M_HC, i=i, V=V )
